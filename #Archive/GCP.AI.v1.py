...
import numpy as np
import os
from tkinter import Tk, filedialog, Label, Button, Scale, HORIZONTAL, Text, END, messagebox, Toplevel
...
class ImagePatternMatcher:
    ...
    def display_matches_grid(self):
        ...
        for idx, (score, loc, scale_pattern) in enumerate(self.match_candidates[:6]):  # Show top 6 matches
            ...
            match_label.bind("<Button-1>", lambda event, idx=idx: self.store_feedback(self.match_candidates[idx]))

        grid_window.mainloop()

    def store_feedback(self, selected_match):

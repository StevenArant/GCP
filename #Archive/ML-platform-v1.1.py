...
def generate_data(n_samples, img_size):
    ...
    for _ in range(n_samples):
        ...
        if np.random.random() > 0.5:
            ...
            checker = create_checkerboard(np.random.randint(10, 50), angle)
...
def train_model():
    ...
    global X_train, y_train
    X_train, y_train, centers = generate_data(1000, 64)
...
def show_training_images():
    ...
    X_sample, y_sample, centers = generate_data(9, 64)
...
def upload_and_predict():
    ...
    if file_path:
        ...
        center_x, center_y = 32, 32  # Assuming the center is the middle of the resized image
        
        # Display the image with a red 'X' marking the checkerboard center
        fig, ax = plt.subplots()
        ax.imshow(img_resized[0].reshape(64, 64), cmap='gray')
        if prediction > 0.5:
        ...
        confirmed = messagebox.askyesno("Prediction Result", f"{label}. Confirm?")
        
        if confirmed:
            add_to_training(img_resized, 1 if prediction > 0.5 else 0)
        else:
            add_to_training(img_resized, 0 if prediction > 0.5 else 1)
